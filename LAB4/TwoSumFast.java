...
public class TwoSumFast {
	...
	public static int count(int[] a) {//constant time operation...O(1)
		...
		Arrays.sort(a);//this is a O(log n) operation
		if (containsDuplicates(a)) throw new IllegalArgumentException("array contains duplicate integers");//constant time operation...O(1)
		int count = 0;//constant time operation...O(1)
		for (int i = 0; i < n; i++) {//runs n times...O(n)
			int j = Arrays.binarySearch(a, -a[i]);//runs logn times thus total is O(nlogn)
			if (j > i) count++;//constant time operation...O(1)
		}
		return count;//constant time operation...O(1)
	} 
//adding all the O(1)+O(1)+O(1)+O(1)+O(n)+O(nlogn)+O(n^2) gives O(5+n+nlogn+logn)
//ignoring constants gives us O(nlogn)

...
public class ThreeSum {
    ...
    public static int count(int[] a) {
        ...
        for (int i = 0; i < n; i++) {//in total, this loops n^3 times, thus, O(n^3)
            for (int j = i+1; j < n; j++) {//loops n-1 times...O(n-1)
                for (int k = j+1; k < n; k++) {//loops n-2 times...O(n-2)
                    if (a[i] + a[j] + a[k] == 0) {//constant time operation...O(1)
    ...
    //adding O(1)+O(1)+O(1)+O(1)+O(1)+O((n-1)(n-2)(n)) gives O(n^3)...ignoring constants
    //therefore, it is an O(n^3) operation

...
public class ThreeSumFast {
    ...
    public static int count(int[] a) {
        ...
        Arrays.sort(a);//runs logn times
        if (containsDuplicates(a)) throw new IllegalArgumentException("array contains duplicate integers");//runs n times...O(n)
        int count = 0;//constant time operation...O(1)
        for (int i = 0; i < n; i++) {//in total, this method loops O(n^2*(logn))
            for (int j = i+1; j < n; j++) {//loops n-1 times, O(n-1) 
                int k = Arrays.binarySearch(a, -(a[i] + a[j]));//loops logn times, O(logn)
                if (k > j) count++;//constant time operation...O(1)
            }
        }
        return count;//constant time operation...O(1)

    } 
//thus, in total, this is a O(n^2*logn) algorithms (ignoring constants)
 /**
     * 
